# Histogram-Anova-v3.3
# Author: Niels Asmussen
# Date: February 2019
##########################################
####################################################################################
#
# dump out some values...
####################################################################################
...
levels(raw.summary[,'Group1']) = c('NT', 'NC', '22', '122', '223', '451')
levels(raw[,'Group1'])
...
#
# To Do:
# - Better display / sizing of the figure for export & saving of output
# - auto guess the YMax value (using upper in the summary table) and the Stat Offset for variable letter sizes
# - handle greek letters in the config file for titles
...
init_vars <- function() {
  ...
  assign("Stats.Test", c("ANOVA"), envir = .GlobalEnv)
...
load_file_head = function() {
  ...
  while(substring(l, 1, 1) == '#') {
    ...
    else if (lA[[1]][1] == "X Leg") { assign("Fig.X", lA[[1]][2], envir = .GlobalEnv) }
    else if (lA[[1]][1] == "Y Leg") { assign("Fig.Y", lA[[1]][2], envir = .GlobalEnv) }
    else if (lA[[1]][1] == "Title Size") { assign("Fig.Title.Size", lA[[1]][2], envir = .GlobalEnv) }
    ...
    else if (lA[[1]][1] == "HLine") { assign("HLine", as.numeric(lA[[1]][2]), envir = .GlobalEnv) }
    # can handle "," and " " for splitting ([, ]+ looks for "," or " " multiple times for splitting)
      ...
      else if (lA[[1]][2] %in% c("STTest", "sttest", "STtest")) { Stats.Test = c(Stats.Test, "STTest") }
      else if (lA[[1]][2] %in% c("PTTest", "pttest", "PTtest", "Pttest")) { Stats.Test = c(Stats.Test, "PTTest") }
      assign("Stats.Test", Stats.Test, envir = .GlobalEnv)
    }
    else if (lA[[1]][1] == "Stats STTest Pairs") { 
      STTest.tails = "two.sided"
      if (length(lA[[1]]) > 4) { 
        if (lA[[1]][5] %in% c("two.sided", "greater", "less")) { STTest.tails = lA[[1]][5] }
        else { warning(sprintf("---- Argument in STTest (%s) NOT VALID, using default (%s) instead", lA[[1]][5], STTest.tails)) }
      }
      assign("Stats.STTest.Pairs", rbind(Stats.STTest.Pairs, data.frame(g1 = lA[[1]][2], g2 = lA[[1]][3], l = lA[[1]][4], alt = STTest.tails, ftest = I(vector(mode="list", length=1)), sttest = I(vector(mode="list", length=1))) ), envir = .GlobalEnv)
    }
    else if (lA[[1]][1] == "Stats PTTest Pairs") { 
      PTTest.tails = "two.sided"
      if (length(lA[[1]]) > 4) { 
        if (lA[[1]][5] %in% c("two.sided", "greater", "less")) { PTTest.tails = lA[[1]][5] }
        else { warning(sprintf("---- Argument in PTTest (%s) NOT VALID, using default (%s) instead", lA[[1]][5], PTTest.tails)) }
      }
      assign("Stats.PTTest.Pairs", rbind(Stats.PTTest.Pairs, data.frame(g1 = lA[[1]][2], g2 = lA[[1]][3], l = lA[[1]][4], alt = PTTest.tails, pttest = I(vector(mode="list", length=1))) ), envir = .GlobalEnv)
    }
  ...
  # Create the title for the actual figure
  Fig.Title <- substitute(
    paste(bold(m)),
    list(m = Fig.Title.tmp)
  )
  assign("Fig.Title", Fig.Title, envir = .GlobalEnv)
  
  if (exists("Fig.Y.Replace")) { 
    assign("Fig.Y", Fig.Y.Replace, envir = .GlobalEnv)
    rm(Fig.Y.Replace, envir = .GlobalEnv)
  }
  if (exists("Fig.Title.Replace")) { 
    assign("Fig.Title", Fig.Title.Replace, envir = .GlobalEnv)
...
build_histo <- function(){
  ...
  mynamestheme = theme(plot.title = element_text(family = "Helvetica", face = "bold", size = (18)))
  ...
  gplot = gplot + theme(
            ...
            axis.title.x = element_text(color="black", margin = margin(t = Fig.Axis.LabelSep, r = 0, b = 0, l = 0)),
...
generate_figure <- function() {
  ...
  if (HLine != FALSE) {
    message(sprintf("adding a horizontal line to the figure at: \'%s\'", HLine))
    gplot = gplot + geom_hline(yintercept=HLine, linetype="solid", color="black", size=1)  
    assign("HLine", FALSE, envir = .GlobalEnv)
  }
  gplot
...
#
##########################################
# the following all 'revert' back to specified defaults after each run through generate_figure()
# should a horizontal line be included? (False: NO; #: location of line)
HLine = FALSE
# should a group ONE label be muted from the figure? (False: NO; 'name of group1' = group to drop)
# *** WARNING - ONLY USE w/ STATS ANOVA GROUP2 = FALSE! ***
group1Mute = FALSE
#
##########################################
# File Directory (convert \ to /; does not end with /):
Location.dir = "W:/Boyan Group/Boyan Lab/Niels Asmussen/Statistical Analysis"

# File name
Location.file = "test-Vertical-2_groups.txt"
Location.file = "test-Vertical-TC.txt"
Location.file = "test-Vertical-TimeCourse.txt"

# any figure edits (eg title or axis label edits)
...
# turn off stat reporting on the bottom of the figure
assign("Stats.Caption.Display", FALSE, envir = .GlobalEnv)

Fig.Title.Replace = expression(bold(paste("TGF", beta, " - 48hrs")))
Fig.Title.Replace = expression(bold(atop("Proliferation", paste("Click iT EdU"))))

Fig.Y.Replace = expression(bold(paste(mu, "g/mL DNA")))
Fig.Y.Replace = expression(bold(paste("Sulfated Glycosaminoglycans (", mu, "G per well)")))
Fig.Y.Replace = expression(bold(atop("Sulfated Glycosaminoglycans", paste("(", mu, "G per well)"))))
Fig.Y.Replace = expression(bold(paste(mu, "g / ", mu, "g DNA")))
Fig.Y.Replace = expression(bold(paste("TGF", Beta)))

Fig.Colors = c("#ffffff", "#606060", "#00c000", "#f71480", "#0000ff", "#ff0000")
Fig.Colors = c("#000000", "#606060", "#00c000", "#f71480", "#0000ff", "#ff0000", "#000000", "#606060", "#00c000", "#f71480", "#0000ff", "#ff0000")
Fig.Colors = c("#ffff00", "#000000", "#606060", "#00c000", "#f71480", "#0000ff", "#ff0000")
